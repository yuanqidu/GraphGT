    ...
    def __init__(self, G_list, max_num_nodes, features='id'):
        ...
        for G in G_list:
            ...
            if features == 'id':
                ...
                degs = np.expand_dims(np.pad(degs, [0, max_num_nodes - G.number_of_nodes()], 'constant'),
                                      axis=1)
                ...
                for i in range(len(G.nodes)):
                    current_feature.append(G.nodes[i]['feature'])
                # for item in G:
                #     current_feature.append(item['feature'])
                current_feature = np.array(current_feature)
                self.feature_all.append(current_feature)

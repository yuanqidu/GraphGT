...
class MLP_VAE_plain(nn.Module):
    def __init__(self, h_size, embedding_size, y_size):
        ...
        self.encode_10 = nn.Linear(h_size, embedding_size)
        self.encode_11 = nn.Linear(embedding_size, embedding_size)  # mu
        self.encode_12 = nn.Linear(embedding_size, embedding_size)  # lsgms
        ...
        self.decode_3 = nn.Linear(embedding_size, h_size)
        # self.decode_4 = nn.Linear(embedding_size, 3)
    ...
    def sample(self):
        z = torch.randn((1, self.embedding_size)).cuda()
        y_node = self.decode_3(z)
        y = self.decode_1(z)
        y = self.relu(y)
        y = self.decode_2(y)

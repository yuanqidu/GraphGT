...
class Args_DGMG():
    def __init__(self):
        ...
        ### data config
        self.graph_type = 'random_geometry'
        ...
        self.max_prev_node = 24

        ### network config
        self.node_embedding_size = 16
...
def adjacency_list(g):
    return list(map(list, iter(g.adj.values())))
...
def train_DGMG_epoch(epoch, args, model, dataset, optimizer, scheduler, is_fast = False):
    ...
    for i in order:
        ...
        while node_count<=graph.number_of_nodes():
            node_neighbor = adjacency_list(graph.subgraph(list(range(node_count)))) # list of lists (first node is zero)
            node_neighbor_new = adjacency_list(graph.subgraph(list(range(node_count+1))))[-1] # list of new node's neighbors
            # node_neighbor2 = [(n, nbrdict) for n, nbrdict in node_neighbor.adjacency()]
            # print (node_neighbor.edges)
            # for n, nbrs in node_neighbor.adjacency() :
            #     print (n)
            #     for nbr, edict in nbrs.items():
            #         print (nbr, edict)
            # print (node_neighbor)
            # print (node_neighbor[0][1][22])
            # 1 message passing
            ...
            while edge_count<=len(node_neighbor_new):
                ...
                if edge_count < len(node_neighbor_new):
                    ...
                    # print('node_neighbor_new',node_neighbor_new, edge_count, p_node.size(1))
...
if __name__ == '__main__':
    ...
    input_root_dir = '../random_geometry/RANDOM_GEOMETRY_full/timestep_0'
    dataset_name = 'random_geometry'
